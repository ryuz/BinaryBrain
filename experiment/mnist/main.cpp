...
void MnistCnnBin2(int epoch_size, size_t mini_batch_size, bool binary_mode = true);
void MnistCnnBin3(int epoch_size, size_t mini_batch_size, bool binary_mode = true);
void MnistCnnBin4(int epoch_size, size_t mini_batch_size, bool binary_mode = true);
...
{
	bb::NeuralNetDenseToSparseAffine<>* layer0;
	bb::NeuralNetDenseToSparseAffine<>* layer1;
	bb::NeuralNetDenseToSparseAffine<>* layer2;
};

void callbakMnistMlpDenseToSparse(bb::NeuralNet<>* net, void* user)
{
	auto info = (DenseToSparseInfo*)user;

	*(net->m_log_stream)
		<< std::setw(4) << info->layer0->GetDecimatedInputSize() << ",  "
		<< std::setw(4) << info->layer1->GetDecimatedInputSize() << ",  "
		<< std::setw(4) << info->layer2->GetDecimatedInputSize() << std::endl;
...
void MnistMlpDenseToSparse(int pre_epoch_size, int epoch_size, size_t max_batch_size, bool binary_mode)
{
	...
	// load MNIST data
	auto td = bb::LoadMnist<>::Load();	//  (10, 512, 512);
	...
	// build layer
	bb::NeuralNetDenseToSparseAffine<>	layer0_affine(6, 0.9999, input_node_size*input_hmux_size, layer0_node_size);
	bb::NeuralNetReLU<>					layer0_act(layer0_node_size);
	bb::NeuralNetDenseToSparseAffine<>	layer1_affine(6, 0.9999, layer0_node_size, layer1_node_size);
	bb::NeuralNetReLU<>					layer1_act(layer1_node_size);
	bb::NeuralNetDenseToSparseAffine<>	layer2_affine(6, 0.9999, layer1_node_size, layer2_node_size);
//	bb::NeuralNetSparseAffine<6>		layer0_affine(input_node_size*input_hmux_size, layer0_node_size);
//	bb::NeuralNetSparseAffine<6>		layer1_affine(layer0_node_size, layer1_node_size);
//	bb::NeuralNetSparseAffine<6>		layer2_affine(layer1_node_size, layer2_node_size);
	bb::NeuralNetGroup<>		bin_group;
	bin_group.AddLayer(&layer0_affine);
	bin_group.AddLayer(&layer0_act);
	bin_group.AddLayer(&layer1_affine);
	bin_group.AddLayer(&layer1_act);
	bin_group.AddLayer(&layer2_affine);

	DenseToSparseInfo inf;
	inf.layer0 = &layer0_affine;
	inf.layer1 = &layer1_affine;
	inf.layer2 = &layer2_affine;
	...
	if (0) {
		...
		// 読み込み
		net.Fitting(run_name, td, 0, max_batch_size, &acc_func, &loss_func, true, true);

		int hist_2d[28][28] = { 0 };

		auto input_size  = layer0_affine.GetInputNodeSize();
		auto output_size = layer0_affine.GetOutputNodeSize();
		for (bb::INDEX i = 0; i < output_size; ++i) {
			for (bb::INDEX j = 0; j < input_size; ++j) {
				auto w = layer0_affine.W(i, j);
				if ( w != 0) {
					std::cout << j << ",";
					hist_2d[j / 28][j % 28]++;
				}
	...
	if (1) {
		// プレ学習
		if (0) {
			layer0_affine.SetDecimatedInputSize(6);
			layer1_affine.SetDecimatedInputSize(6);
			layer2_affine.SetDecimatedInputSize(6);
			layer0_affine.SetDecimateEnable(true);
			layer1_affine.SetDecimateEnable(true);
			layer2_affine.SetDecimateEnable(true);
		}
		else {
			layer0_affine.SetDecimateEnable(false);
		...
		// test
		{
			float hist_2d[28][28] = { 0 };

			auto input_size = layer0_affine.GetInputNodeSize();
			auto output_size = layer0_affine.GetOutputNodeSize();
			for (bb::INDEX i = 0; i < output_size; ++i) {
				for (bb::INDEX j = 0; j < input_size; ++j) {
	//				auto n = layer0_affine.m_N(j, i);
					auto e = layer0_affine.m_ErrSum(j, i);
	//				std::cout << j << ",";
					hist_2d[j / 28][j % 28] += sqrt(e);// / n;
				}
				std::cout << std::endl;
			}

			{
				std::ofstream ofs("hist_2d_dx.txt");
				for (int i = 0; i < 28; ++i) {
					for (int j = 0; j < 28; ++j) {
						ofs << hist_2d[i][j] << "\t";
					}
	...
	std::cout << "DecimateEnable" << std::endl;
	layer0_affine.SetDecimateEnable(true);
	layer1_affine.SetDecimateEnable(true);
	layer2_affine.SetDecimateEnable(true);
...
//	return 0;
	...
	/*
	MnistDenseSimpleConvolution3x3(256, 128, true,  true,  "MnistDenseSimpleConvolution3x3_binact_binin");

//	MnistMlpLutDirect(16, 8192, "MnistMlpLutDirect");
	MnistMlpLutN<64>(16, 128, true, false, "MnistMlpLutN_64");
	MnistMlpLutN<32>(16, 128, true, false, "MnistMlpLutN_32");
	MnistMlpLutN<16>(16, 128, true, false, "MnistMlpLutN_16");
	MnistMlpLutN<8> (16, 128, true, false, "MnistMlpLutN_8");
	MnistMlpLutN<4>(16, 128, true, false, "MnistMlpLutN_4");
	MnistMlpLutDirect(16, 8192, "MnistMlpLutDirect");
	getchar();
	return 0;
	*/
...
//	bb::NeuralNetDropout<>					layer0_dropout(32 * 26 * 26, 0.5, 1);
	...
	bb::NeuralNet<> net;
	net.AddLayer(&layer0_conv);
	if (binary_mode) { net.AddLayer(&layer0_norm); }
	net.AddLayer(&layer0_act);
	net.AddLayer(&layer1_conv);
	if (binary_mode) { net.AddLayer(&layer1_norm); }
	net.AddLayer(&layer1_act);
	net.AddLayer(&layer2_maxpol);
//	net.AddLayer(&layer2_dropout);
	net.AddLayer(&layer3_affine);
	if (binary_mode) { net.AddLayer(&layer3_norm); }
	net.AddLayer(&layer3_act);
//	net.AddLayer(&layer3_dropout);
	net.AddLayer(&layer4_affine);
	if (binary_mode) {
...
// MNIST CNN with LUT networks
void MnistCnnBin2(int epoch_size, size_t mini_batch_size, bool binary_mode)
{
	// run name
	std::string run_name = "MnistCnnBin2";
	int			num_class = 10;
	...
	// sub-networks for convolution(3x3)
	bb::NeuralNetSparseMicroMlp<6, 16>	sub0_smm0(1 * 3 * 3, 384);
	bb::NeuralNetSparseMicroMlp<6, 16>	sub0_smm1(384, 64);
	bb::NeuralNetGroup<>				sub0_net;
	sub0_net.AddLayer(&sub0_smm0);
	sub0_net.AddLayer(&sub0_smm1);

	// sub-networks for convolution(3x3)
	bb::NeuralNetSparseMicroMlp<6, 16>	sub1_smm0(64 * 3 * 3, 384);
	bb::NeuralNetSparseMicroMlp<6, 16>	sub1_smm1(384, 64);
	bb::NeuralNetGroup<>		sub1_net;
	sub1_net.AddLayer(&sub1_smm0);
	sub1_net.AddLayer(&sub1_smm1);

	// sub-networks for convolution(3x3)
	bb::NeuralNetSparseMicroMlp<6, 16>	sub3_smm0(64 * 3 * 3, 384);
	bb::NeuralNetSparseMicroMlp<6, 16>	sub3_smm1(384, 64);
	bb::NeuralNetGroup<>				sub3_net;
	sub3_net.AddLayer(&sub3_smm0);
	sub3_net.AddLayer(&sub3_smm1);

	// sub-networks for convolution(3x3)
	bb::NeuralNetSparseMicroMlp<6, 16>	sub4_smm0(64 * 3 * 3, 384);
	bb::NeuralNetSparseMicroMlp<6, 16>	sub4_smm1(384, 64);
	bb::NeuralNetGroup<>				sub4_net;
	sub4_net.AddLayer(&sub4_smm0);
	sub4_net.AddLayer(&sub4_smm1);
	...
	bb::NeuralNetSparseMicroMlp<6, 16>	layer6_smm(64 * 4 * 4, 480);
	bb::NeuralNetSparseMicroMlp<6, 16>	layer7_smm(480, 80);
	bb::NeuralNetBinaryToReal<float>	output_bin2real(80, 10);
...
// MNIST CNN with LUT networks
void MnistCnnBin3(int epoch_size, size_t mini_batch_size, bool binary_mode)
{
	// run name
	std::string run_name = "MnistCnnBin3";
	int			num_class = 10;
	...
	// sub-networks for convolution(3x3)
	bb::NeuralNetSparseMicroMlp<6, 16>	sub0_smm0(1 * 3 * 3, 512);
	bb::NeuralNetSparseMicroMlp<6, 16>	sub0_smm1(512, 128);
	bb::NeuralNetGroup<>				sub0_net;
	sub0_net.AddLayer(&sub0_smm0);
	sub0_net.AddLayer(&sub0_smm1);

	// sub-networks for convolution(3x3)
	bb::NeuralNetSparseMicroMlp<6, 16>	sub1_smm0(128 * 3 * 3, 512);
	bb::NeuralNetSparseMicroMlp<6, 16>	sub1_smm1(512, 512);
	bb::NeuralNetSparseMicroMlp<6, 16>	sub1_smm2(512, 128);
	bb::NeuralNetGroup<>		sub1_net;
	sub1_net.AddLayer(&sub1_smm0);
	sub1_net.AddLayer(&sub1_smm1);
	...
	// sub-networks for convolution(3x3)
	bb::NeuralNetSparseMicroMlp<6, 16>	sub3_smm0(128 * 3 * 3, 512);
	bb::NeuralNetSparseMicroMlp<6, 16>	sub3_smm1(512, 512);
	bb::NeuralNetSparseMicroMlp<6, 16>	sub3_smm2(512, 128);
	bb::NeuralNetGroup<>				sub3_net;
	sub3_net.AddLayer(&sub3_smm0);
	sub3_net.AddLayer(&sub3_smm1);
	...
	// sub-networks for convolution(3x3)
	bb::NeuralNetSparseMicroMlp<6, 16>	sub4_smm0(128 * 3 * 3, 512);
	bb::NeuralNetSparseMicroMlp<6, 16>	sub4_smm1(512, 512);
	bb::NeuralNetSparseMicroMlp<6, 16>	sub4_smm2(512, 128);
	bb::NeuralNetGroup<>				sub4_net;
	sub4_net.AddLayer(&sub4_smm0);
	sub4_net.AddLayer(&sub4_smm1);
	...
	bb::NeuralNetRealToBinary<float>	input_real2bin(28 * 28, 28 * 28);
	bb::NeuralNetLoweringConvolution<>	layer0_conv(&sub0_net, 1, 28, 28, 128, 3, 3);
	bb::NeuralNetLoweringConvolution<>	layer1_conv(&sub1_net, 128, 26, 26, 128, 3, 3);
	bb::NeuralNetMaxPooling<>			layer2_maxpol(128, 24, 24, 2, 2);
	bb::NeuralNetLoweringConvolution<>	layer3_conv(&sub3_net, 128, 12, 12, 128, 3, 3);
	bb::NeuralNetLoweringConvolution<>	layer4_conv(&sub4_net, 128, 10, 10, 128, 3, 3);
	bb::NeuralNetMaxPooling<>			layer5_maxpol(128, 8, 8, 2, 2);
	bb::NeuralNetSparseMicroMlp<6, 16>	layer6_smm(128 * 4 * 4, 512);
	bb::NeuralNetSparseMicroMlp<6, 16>	layer7_smm(512, 256);
	bb::NeuralNetSparseMicroMlp<6, 16>	layer8_smm(256, 80);
	bb::NeuralNetBinaryToReal<float>	output_bin2real(80, 10);
...
void Connect3d(bb::NeuralNetSparseMicroMlp<6, 16>& layer, int c_size, int h_size, int w_size, int n_size, int c_range)
{
	bb::ShuffleSet sset(c_range * h_size * w_size);
	for ( int n = 0; n < n_size; ++n ) {
		auto s = sset.GetRandomSet(6);
		for ( int i = 0; i < 6; ++i ) {
			int c = (s[i] % (h_size * w_size) + n) % c_size;
			int k = s[i] / (h_size * w_size);
			layer.SetNodeInput(n, i, c*(h_size * w_size) + k);
		}
	}
}
...
// MNIST CNN with LUT networks
void MnistCnnBin4(int epoch_size, size_t mini_batch_size, bool binary_mode)
{
	// run name
	std::string run_name = "MnistCnnBin4";
	int			num_class = 10;
	...
	sub1_net.AddLayer(&sub1_smm1);
	sub1_net.AddLayer(&sub1_smm2);
	Connect3d(sub1_smm0, 128, 3, 3, 128, 3);
	...
	sub3_net.AddLayer(&sub3_smm1);
	sub3_net.AddLayer(&sub3_smm2);
	Connect3d(sub3_smm0, 128, 3, 3, 128, 3);
	...
	sub4_net.AddLayer(&sub4_smm1);
	sub4_net.AddLayer(&sub4_smm2);
	Connect3d(sub4_smm0, 128, 3, 3, 128, 3);
