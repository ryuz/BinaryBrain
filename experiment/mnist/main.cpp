...
// Real DenseAffine network
void MnistDenseAffineSigmoid(int epoc_size, size_t max_batch_size)
{
	// run name
	std::string run_name = "MnistDenseAffineSigmoid";
	int			num_class = 10;
...
// LUT6入力のバイナリ版の力技学習
void BinaryLut6WithBbruteForce(int epoc_size, size_t max_batch_size)
{
	// run name
	std::string run_name = "BinaryLut6WithBbruteForce";
	int			num_class = 10;
	int			max_train = -1;
	int			max_test = -1;
	...
	size_t output_hmux_size = 32;

	size_t layer0_node_size = 360 * output_hmux_size;
	...
	auto test_accuracy = net.RunCalculation(train_data.x_test, train_data.y_test, max_batch_size, &accFunc);
	...
	//			;
			while (bin_mux.Feedback(bin_mux.CalcLoss<std::uint8_t, 10>(y_train, train_index)))
				;

			// 途中評価
			bin_mux.SetMuxSize(test_mux_size);	// 評価用の多重化数にスイッチ
			auto test_accuracy = net.RunCalculation(x_test, y_test, max_batch_size, &accFunc);
		...
		bin_mux.SetMuxSize(test_mux_size);	// 評価用の多重化数にスイッチ
		auto test_accuracy = net.RunCalculation(x_test, y_test, max_batch_size, &accFunc);
		auto train_accuracy = net.RunCalculation(x_train,y_train, max_batch_size, &accFunc);
