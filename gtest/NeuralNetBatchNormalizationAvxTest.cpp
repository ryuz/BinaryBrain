...
//	batch_norm.Forward(false);
	...
	*/
	...
	std::cout << out_val.GetReal(1, 0) << std::endl;
	std::cout << out_val.GetReal(2, 0) << std::endl;
	std::cout << out_val.GetReal(3, 0) << std::endl;
	std::cout << out_val.GetReal(4, 0) << std::endl;
	std::cout << out_val.GetReal(5, 0) << std::endl;
	std::cout << out_val.GetReal(6, 0) << std::endl;
	std::cout << out_val.GetReal(7, 0) << std::endl;
#endif

	// _mm256_rsqrt_ps ‚ðŽg‚Á‚Ä‚¢‚é‚Ì‚Å¸“x‚Íˆ«‚¢
	EXPECT_TRUE(abs(out_val.GetReal(0, 0) - -1.52752510) < 0.001);
	EXPECT_TRUE(abs(out_val.GetReal(1, 0) - -1.09108940) < 0.001);
	EXPECT_TRUE(abs(out_val.GetReal(2, 0) - -0.65465360) < 0.001);
	EXPECT_TRUE(abs(out_val.GetReal(3, 0) - -0.21821786) < 0.001);
	EXPECT_TRUE(abs(out_val.GetReal(4, 0) - +0.21821786) < 0.001);
	EXPECT_TRUE(abs(out_val.GetReal(5, 0) - +0.65465360) < 0.001);
	EXPECT_TRUE(abs(out_val.GetReal(6, 0) - +1.09108940) < 0.001);
	EXPECT_TRUE(abs(out_val.GetReal(7, 0) - +1.52752510) < 0.001);

	EXPECT_TRUE(abs(out_val.GetReal(0, 1) - -1.23359570) < 0.001);
	EXPECT_TRUE(abs(out_val.GetReal(1, 1) - +1.14442010) < 0.001);
	EXPECT_TRUE(abs(out_val.GetReal(2, 1) - -0.04458780) < 0.001);
	EXPECT_TRUE(abs(out_val.GetReal(3, 1) - -0.63909180) < 0.001);
	EXPECT_TRUE(abs(out_val.GetReal(4, 1) - -1.11469500) < 0.001);
	EXPECT_TRUE(abs(out_val.GetReal(5, 1) - +1.62002340) < 0.001);
	EXPECT_TRUE(abs(out_val.GetReal(6, 1) - +0.78771776) < 0.001);
	EXPECT_TRUE(abs(out_val.GetReal(7, 1) - -0.52019095) < 0.001);
