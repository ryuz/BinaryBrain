...
void MnistDenseAffineQuantize(int epoch_size, int mini_batch_size, int train_modulation_size, int test_modulation_size, bool binary_mode, bool file_read)
{
...

    {
        ...
        main_net->Add(bb::DenseAffineQuantize<float>::Create(1024, 3, ob, ib, 1.0f/(1<<3), 1.0f/(1<<oq), 1.0/(1 << iq)));
//        main_net->Add(bb::BatchNormalization<float>::Create());
        main_net->Add(bb::ReLU<float>::Create());
        main_net->Add(bb::DenseAffineQuantize<float>::Create(512, wb, ob, ib, 1.0f/(1<<wq), 1.0f/(1<<oq), 1.0/(1 << iq)));
//        main_net->Add(bb::BatchNormalization<float>::Create());
        main_net->Add(bb::ReLU<float>::Create());
        main_net->Add(bb::DenseAffineQuantize<float>::Create(td.t_shape, wb, ob, ib, 1.0f/(1<<wq), 1.0f/(1<<oq), 1.0/(1 << iq)));
        if ( binary_mode ) {
//            main_net->Add(bb::BatchNormalization<float>::Create());
            main_net->Add(bb::ReLU<float>::Create());
        }
