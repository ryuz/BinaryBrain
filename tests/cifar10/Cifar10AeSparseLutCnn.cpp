...
static void WritePpm(std::string fname, bb::FrameBuffer buf, int frame)
{
    ...
    for ( int i = 0; i < 32*32; ++i ) {
        ofs << (int)(buf.GetFP32(frame, (32*32)*0+i) * 255.0f) << " ";
        ofs << (int)(buf.GetFP32(frame, (32*32)*1+i) * 255.0f) << " ";
        ofs << (int)(buf.GetFP32(frame, (32*32)*2+i) * 255.0f) << "\n";
    }
}
...
void Cifar10AeSparseLutCnn_Tmp(int epoch_size, int mini_batch_size, int train_modulation_size, int test_modulation_size, bool binary_mode, bool file_read)
{
  ...
  // load cifar-10 data
...

    {
        ...
        WritePpm("out_0y.ppm", y_buf, 0);
        WritePpm("out_1x.ppm", x_buf, 1);
        WritePpm("out_1y.ppm", y_buf, 1);
        WritePpm("out_2x.ppm", x_buf, 2);
        WritePpm("out_2y.ppm", y_buf, 2);
        WritePpm("out_3x.ppm", x_buf, 3);
        WritePpm("out_3y.ppm", y_buf, 3);
        WritePpm("out_4x.ppm", x_buf, 4);
        WritePpm("out_4y.ppm", y_buf, 4);
    }
