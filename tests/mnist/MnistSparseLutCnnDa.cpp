...
void DataAugmentationProc(bb::TrainData<>& td, std::uint64_t seed, void *user)
{
...
void MnistDifferentiableLutCnnDa(int epoch_size, int mini_batch_size, int train_modulation_size, int test_modulation_size, bool binary_mode, bool file_read)
{
  ...
  // load MNIST data
    ...
    // その他のために11分類に変更
//  td.t_shape[0] += 1;
//  for ( auto& t : td.t_train ) { t.push_back(0); }
//  for ( auto& t : td.t_test  ) { t.push_back(0); }

#if 0
    // 変形
    std::mt19937_64 mt(2);
    for ( auto& x : td.x_train ) { ImageAugmentation<>(x, mt); }
    for ( auto& x : td.x_test  ) { ImageAugmentation<>(x, mt); }

    // はみ出したものは無効判定
    auto mask = MakeMaskImage<>();
    for ( size_t i = 0; i < td.x_train.size(); ++i ) {
        if ( !CheckMask<>(td.x_train[i], mask) ) {
            for ( auto& t : td.t_train[i] ) { t = 0; }
            *td.t_train[i].rbegin() = 1.0f;
        }
    }
    for ( size_t i = 0; i < td.x_test.size(); ++i ) {
        if ( !CheckMask<>(td.x_test[i], mask) ) {
            for ( auto& t : td.t_test[i] ) { t = 0; }
            *td.t_test[i].rbegin() = 1.0f;
        }
    }
#endif
...

    {
        ...
        main_net->Add(bb::Convolution2d<bb::Bit>::Create(cnv1_sub, 3, 3));
        main_net->Add(bb::MaxPooling<bb::Bit>::Create(2, 2));
        main_net->Add(bb::Convolution2d<bb::Bit>::Create(cnv2_sub, 3, 3));
        main_net->Add(bb::Convolution2d<bb::Bit>::Create(cnv3_sub, 3, 3));
        main_net->Add(bb::MaxPooling<bb::Bit>::Create(2, 2));
