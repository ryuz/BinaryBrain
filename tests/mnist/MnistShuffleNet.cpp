...
void MnistShuffleNet_(int epoch_size, int mini_batch_size, int train_modulation_size, int test_modulation_size, bool binary_mode, bool file_read)
{
...

    {
        ...
        main_net->Add(bb::Convolution2d<T>::Create(MakePointwiseLayer<T>({1, 1,  32*w}, 8), 1, 1));
        main_net->Add(bb::Convolution2d<T>::Create(MakeDepthwiseLayer<T>({1, 1,  32*w}), 3, 3));  // 26x26
        main_net->Add(bb::Convolution2d<T>::Create(MakePointwiseLayer<T>({1, 1,  32  }, shuffle_unit), 1, 1));

        main_net->Add(bb::Convolution2d<T>::Create(MakePointwiseLayer<T>({1, 1,  64*w}, shuffle_unit), 1, 1));
        main_net->Add(bb::Convolution2d<T>::Create(MakeDepthwiseLayer<T>({1, 1,  64*w}), 3, 3));  // 24x24
        main_net->Add(bb::Convolution2d<T>::Create(MakePointwiseLayer<T>({1, 1,  64  }, shuffle_unit), 1, 1));
        ...
        main_net->Add(bb::Convolution2d<T>::Create(MakePointwiseLayer<T>({1, 1, 128*w}, shuffle_unit), 1, 1));
        main_net->Add(bb::Convolution2d<T>::Create(MakeDepthwiseLayer<T>({1, 1, 128*w}), 3, 3));  // 8x8
        main_net->Add(bb::Convolution2d<T>::Create(MakePointwiseLayer<T>({1, 1, 128  }, shuffle_unit), 1, 1));
        ...
        main_net->Add(bb::Convolution2d<T>::Create(MakePointwiseLayer<T>({1, 1, 256*w}, shuffle_unit), 1, 1));
        main_net->Add(bb::Convolution2d<T>::Create(MakeDepthwiseLayer<T>({1, 1, 256*w}), 2, 2));  // 1x1
        main_net->Add(bb::Convolution2d<T>::Create(MakePointwiseLayer<T>({1, 1, 256  }, shuffle_unit), 1, 1));

        main_net->Add(bb::Convolution2d<T>::Create(MakePointwiseLayer<T>({1, 1, 512  }, shuffle_unit), 1, 1));
        main_net->Add(bb::Convolution2d<T>::Create(MakePointwiseLayer<T>({1, 1, 512  }, shuffle_unit), 1, 1));
        main_net->Add(bb::Convolution2d<T>::Create(MakePointwiseLayer<T>({1, 1, 256  }, shuffle_unit), 1, 1));
        main_net->Add(bb::Convolution2d<T>::Create(MakePointwiseLayer<T>({1, 1, 128  }, shuffle_unit), 1, 1));
        main_net->Add(bb::Convolution2d<T>::Create(MakePointwiseLayer<T>({1, 1, 64   }, shuffle_unit), 1, 1));
