...
int argmax_img(std::vector<float> const &img, int pix, int pix_size=56*56, int ch_size=11)
{
    float max_val = 0;
    int   max_c   = 0;
...
void write_ppm(std::string filename, std::vector<float> const &img, int w=2*28, int h=2*28)
{
...
void write_ppm(std::string filename, bb::FrameBuffer const &buf, int frame, int w=2*28, int h=2*28)
{
...
std::shared_ptr<bb::Model> make_cnv(int ch_size)
{
