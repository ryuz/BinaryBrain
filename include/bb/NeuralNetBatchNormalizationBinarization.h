...
class NeuralNetBatchNormalizationBinarization : public NeuralNetLayerBuf<T, INDEX>
{
protected:
...
public:
	NeuralNetBatchNormalizationBinarization() {}

	NeuralNetBatchNormalizationBinarization(INDEX node_size, const NeuralNetOptimizer<T, INDEX>* optimizer = nullptr)
	{
...
//					for (int frame = 0; frame < mm256_frame_size; frame += 8) {
					...
					__m256 x = _mm256_load_ps(&x_ptr[frame]);
						...
						// Binarization
						y = _mm256_and_ps(ones, _mm256_cmp_ps(y, zeros, _CMP_GT_OS));
...
//				for (int frame = 0; frame < mm256_frame_size; frame += 8) {
				for (int frame = mm256_frame_size - 8; frame >= 0; frame -= 8) {
					...
					// hard-tanh
					dy = _mm256_and_ps(dy, _mm256_cmp_ps(y, zeros, _CMP_GT_OS));
					dy = _mm256_and_ps(dy, _mm256_cmp_ps(y, ones, _CMP_LT_OS));
