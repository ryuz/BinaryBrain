...
しかしながら、この方法では深いネットワークの学習が指数関数時間となってしまい、実質的に計算できなくなります。
そこで、LUTに対応するパーセプトロンを使った逆伝播可能なネットワークを定義することで、ある程度の高効率性を保ったまま、深いネットワークをFPGA化します。
...
LUTと等価の表現能力を有する多層パーセプトロンの最小単位をmini-MLPと呼ぶことにします。
