...
template <typename T>
__global__ void kernal_PopcountLutN_Forward(
            T   const   *x_buf,
            T           *y_buf,
        ...
        )
{
    ...
    if ( node < node_size && frame < frame_size ) {
        ...
        y_buf[node*frame_stride + frame] = (count > 0) ? (T)BB_BINARY_HI : (T)BB_BINARY_LO;
    }
}
        ...
        )
{
    ...
    kernal_PopcountLutN_Forward<T><<<grid, block, 0, streamId>>>(
            dev_x_buf,
            dev_y_buf,
            dev_input_index,
            n,
...
template BBCU_DLL_EXPORT int bbcu_PopcountLutN_Forward<float >(float  const *, float  *, int const *, int, int, int, int, cudaStream_t);
template BBCU_DLL_EXPORT int bbcu_PopcountLutN_Forward<double>(double const *, double *, int const *, int, int, int, int, cudaStream_t);



__global__ void kernal_bit_PopcountLutN_Forward(
            int const   *x_buf,
            int         *y_buf,
            int const   *input_index,
            int         n,
        ...
        )
{
    ...
    kernal_bit_PopcountLutN_Forward<<<grid, block, 0, streamId>>>(
            dev_x_buf,
            dev_y_buf,
            dev_input_index,
            n,
...
template <typename T>
__global__ void kernal_PopcountLutN_Backward(
            T   const   *dy_buf,
            T           *dx_buf,
            int const   *input_index,
            int         n,
        ...
        )
{
    ...
    kernal_PopcountLutN_Backward<T><<<grid, block, 0, streamId>>>(
            dev_dy_buf,
            dev_dx_buf,
            dev_input_index,
            n,
...
template BBCU_DLL_EXPORT int bbcu_PopcountLutN_Backward<float >(float  const *, float  *, int const *, int, int, int, int, int, cudaStream_t);
template BBCU_DLL_EXPORT int bbcu_PopcountLutN_Backward<double>(double const *, double *, int const *, int, int, int, int, int, cudaStream_t);
